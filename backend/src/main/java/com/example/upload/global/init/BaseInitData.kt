...
class BaseInitData(
    ...
    fun postInit() {
        ...
        val user1: Member = memberService.findByUsername("user1").get()
        val user2: Member = memberService.findByUsername("user2").get()

        val post1: Post = postService.write(user1, "축구 하실분 모집합니다.", "저녁 6시까지 모여주세요.", true, true)
        post1.addComment(user1, "저 참석하겠습니다.")
        post1.addComment(user2, "공격수 자리 있나요?")

        val post2: Post = postService.write(user1, "농구하실분?", "3명 모집", true, false)
        ...
        val post10: Post = postService.write(
                user2,
        "테니스 하실 분있나요?",
        "테니스 강력 추천합니다.",
        true,
